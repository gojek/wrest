...
module Wrest
  class CallbackBuilder
    def initialize(callbacks={})
      ...
      @callbacks.each do |status, callback|
        @callbacks[status] = [callback] unless callback.is_a?(Array)
      end
    end
    ...
    def execute_callbacks(response)
      @callbacks.each do |callback_response_range, callback_list|
        callback_list.each {|callback| callback.call(response)} if case callback_response_range
        when Range
          callback_response_range.include?(response.code.to_i)
        when Fixnum
          callback_response_range == response.code.to_i
        end
      end
    end
  end
