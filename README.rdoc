...
Since no single framework can provide an object oriented wrapper suitable for _all_ available web services, it follows that Wrest should focus on providing you with the tools to help you roll your own. Wrest::Resource is an example of this - an object oriented wrapper for the kind of REST APIs exposed by Rails applications, that you would otherwise use ActiveResource to consume.
...
* Provides infrastructure components such as convenient HTTP wrappers api, serialisation, deserialisation with caching, redirect handling in the works.
