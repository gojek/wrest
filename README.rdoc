...
* Pick your HTTP: use Net::HTTP or LibCurl
* Pick your Ruby: use 1.8.7, 1.9.2, JRuby and Rubinius ({Continuous Integration server}[http://ci.c42.in])
* RFC 2616 based {caching}[https://github.com/kaiwren/wrest/blob/caching/Caching.markdown]
* [Alpha] Go async: use Threads or EventMachine
...
You can change the default to eventmachine.
  ...
  Wrest.always_use_eventmachine_for_asynchronous_requests!

You can also override the default on Uri objects.

  "http://c42.in".to_uri.using_eventmachine.get_async do |callback|
    callback.on_ok do |response|
      Wrest.logger.info "Ok."
    end
  end

Note: The current implementation of asynchronous requests is naive and not production ready.
