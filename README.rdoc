...
* active_support
* ruby-libxml (recommended, will fall back to REXML if absent; be aware that this uses ActiveSupport::XmlMini, so if you're using Wrest as a plugin in a Rails application, all xml parsing will switch to using libxml if available)
