...
The following example will use Memcached to cache the response. This will require Memcached to be already installed. For instructions on how to install Memcached, refer to http://code.google.com/p/memcached/wiki/NewInstallFromPackage
You'll also need to have the Dalli gem to run this.

    Wrest::always_cache_using_hash!     # Hash should NEVER be used in a server. It is unbounded and will keep on increasing in size.
    c42 = "http://c42.in".to_uri.get

You can use the following method to set memcached as the default caching back-end.
    Wrest::always_cache_using_memcached!

For fine-grained control, you can use this API:

    Wrest::enable_memcached_caching!

    uses_cache_store = "http://c42.in".to_uri(:cache_store => Wrest::Components::CacheStore::Memcached.new("localhost:11211"))
    response = c42.get
