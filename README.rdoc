...
* This example does a get on a base uri with several paramers passed to it, resulting in a uri essentially the same as the one above, but returning xml instead of json. It also shows how you can use a custom deserialiser to produce a hash-map from the response. 
