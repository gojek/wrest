...
* This example simply does a get on a uri and figures out the appropriate deserialiser using the content-type (in this case 'text/javascript')
...
* This example does a get on a base uri with several parameters passed to it, resulting in a uri essentially the same as the one above, but returning xml instead of json. It also shows how you can use a custom deserialiser to produce a hash-map from the response. 
