...
module SampleApp
  class Application < Sinatra::Application
    ...
    { get: %w[no_body nothing], post: ['nothing', 'no_bodies.:format'] }.each do |method, paths|
      paths.each do |path|
        send(method, "/#{path}") do
          ' '
        end
    ...
    get '/cacheable/cant_be_validated/with_expires/:seconds_to_cache' do
      ...
      "#{rand(1000)} (random value to identify a fresh response)
    ...
    get '/cacheable/can_be_validated/with_last_modified/always_304/:seconds_to_cache' do
      if request_headers.include? 'if_modified_since'
        ...
        headers 'header-that-changes-everytime' => rand(1..10_000).to_s
        status 304
      else
        headers 'Last-Modified' => Time.now.httpdate
    ...
    get '/query_based_response' do
      headers 'Expires' => (Time.now + (84_600 * 7)).httpdate
      params.to_s
    end
  end
end
